...
def header(work_sheet):
    ...
    work_sheet.write(0, 1, '执行命令', style0)
...
def run_command(item):
    ...
    for cmd in cmds.readlines():
        items = cmd.split('#')
        try:
            work_sheet = work_book.add_sheet(items[0])
        except:
             print ("duplicate worksheet name.",items[0])

        work_sheet = work_book.sheet_by_name(items[0])
        ...
        work_sheet.write(rows,1,items[1])
        work_sheet.write(rows,2,return_info.decode('utf-8'))
